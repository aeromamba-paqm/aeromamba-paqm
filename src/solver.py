...
from src.enhance import enhance, save_wavs, save_specs, run_matlab_process_files
...
from src.utils import bold, copy_state, pull_metric, swap_state, LogProgress, run_ssh_command, save_spectrograms
...
class Solver(object):
            ...
            if ((epoch + 1) % self.eval_every == 0 or epoch == self.epochs - 1) and self.tt_loader:
                ...
                with swap_state(self.model, best_state):
                    ...
                    if evaluated_on_test_data:
                        ...
                        lsd, visqol = evaluate_on_saved_data(self.args, enhanced_dataloader, epoch)
                        if self.args.experiment.paqm_eval: 
                               with torch.no_grad():
                                    iterator = LogProgress(logger, enhanced_dataloader, name="Eval MOS")
                                    mos_eval = []
                                    for i, data in enumerate(iterator):
                                        lr, hr, pr, filename = data
                                        pr_48 = self.resample(pr.squeeze().to(self.args.device))
                                        hr_48 = self.resample(hr.squeeze().to(self.args.device))
                                        ev = PAQM(pr_48, hr_48)
                                        mos_ev = ev.mean_opinion_score
                                        mos_eval.append(mos_ev)
                                    mos_eval = torch.stack(mos_eval)
                                    mos_eval = torch.nanmean(mos_eval).item()
                ...
                if self.args.experiment.paqm_eval:
                    metrics.update({METRICS_KEY_LSD: lsd, METRICS_KEY_MOS: mos_eval})
                else:
    ...
    def _get_paqm_loss(self, pr, hr):
        estimator = PAQM(hr, pr)
        loss = estimator.mean_opinion_score
        print(f'get_paqm_loss: {-loss}')
        return -loss
...
def calc_avg_odg(file_path):
    ...
    with open(file_path, 'r') as file:
        ...
        while i < len(lines):
            ...
            scores.append(score)
            i += 3  # Move to the next filename-score pair
    return sum(scores) / len(scores)

def cp_dir(source, target):
    call(['cp', '-a', source, target]) # Linux

def generate_best_checkpoint(checkpoint_dir, best_odg, content):
    best_odg_file = os.path.join(checkpoint_dir, "best_odg.txt")
    checkpoint_file = os.path.join(checkpoint_dir, "checkpoint.th")
    best_file = os.path.join(checkpoint_dir, "best.th")
    if best_odg > content:
        # best_odg is greater than the content of best_odg.txt
        # Copy checkpoint.th to best.th
        shutil.copyfile(checkpoint_file, best_file)
        # Update best_odg.txt
        with open(best_odg_file, "w") as f:
            f.write(str(best_odg))
    ...
    for n, p in named_parameters:
        if(p.requires_grad) and ("bias" not in n):
            ...
            if grads is not None:
                layers.append(n)
                ave_grads.append(grads.abs().mean().cpu().numpy())
